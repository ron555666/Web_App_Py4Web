...
db.define_table('follow',
                ...
                Field('got_followed_status', 'boolean', default=False))


db.define_table('meow',
...
def add_users_for_testing(num_users):
   ...
   for k in range(num_test_users, num_users):
       ...
       for n in range(3):
           ts -= datetime.timedelta(seconds=random.uniform(300, 90000))
           author_record = db(db.auth_user.username == username).select().first()
           m = dict(
               author=author_record.id,
               timestamp = ts,
           ...
           db.meow.insert(**m)
   db.commit()
for meow in db().select(db.meow.ALL):
